...
- DB は Supabase を主系とし、接続URL、key、Secret、外部URLは必ず `.env` で管理する
...
- `backend/src/repositories`: Supabase へのアクセスを扱う
...
- `.env` を commit しない
- 接続URL、Secret、パスワード、外部URLをコードや `docker-compose.yml` に直書きしない
