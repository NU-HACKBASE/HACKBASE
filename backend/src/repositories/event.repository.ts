...
export class EventRepository {
  ...
  private async calculateEventHeat(eventId: string): Promise<number> {
    ...
    return Math.round(
      heats.reduce((total, heat) => total + heat, 0) / heats.length,
    );
  }
  ...
  private async findChatActivity(eventId: string, userIds: string[]) {
    if (userIds.length === 0) {
      return new Map<
        string,
        { chatCount: number; lastActiveAt: string | null }
      >();
    }
    ...
    for (const row of data ?? []) {
      const current = activity.get(row.user_id) ?? {
        chatCount: 0,
        lastActiveAt: null,
      };
      ...
      activity.set(row.user_id, {
        ...
        lastActiveAt:
          !current.lastActiveAt || createdAt > current.lastActiveAt
            ? createdAt
