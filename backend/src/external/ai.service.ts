...
export class AiService {
  ...
  async generateText(input: GenerateTextInput): Promise<string> {
    ...
    const response = await this.fetchImpl(`${this.apiBaseUrl}/models/${this.model}:generateContent`, {
      method: 'POST',
      headers: {
...
const buildRoomAnalysisPrompt = (input: RoomAnalysisInput) => {
  ...
  return [
    ...
    '- summary は日本語で 80 文字以内。今何が話題か、現地で何が起きているかを自然に要約してください。',
...
const normalizeRoomAnalysis = (value: unknown): RoomAnalysisResult => {
  ...
  const source = value as { heat?: unknown; summary?: unknown; trends?: unknown }
  ...
  const trends = Array.isArray(source.trends)
    ? source.trends
      .filter((trend): trend is string => typeof trend === 'string' && Boolean(trend.trim()))
